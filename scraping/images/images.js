...
const downloadImage = (arr) => {
  const promises = arr.map((item) => {
    const options = {
      url: item,
      dest: path.join(__dirname, `../../public/img/books/${item.split('/').slice(-1)[0]}`),
...
const dropImages = () => {
  fs.readdir(path.join(__dirname, '../../public/img/books'))
    .then(files => {
      if (files !== null) {
        files.map(file => {
          return fs.unlink(path.join(__dirname, `../../public/img/books/${file}`))
        })
      }
    })
